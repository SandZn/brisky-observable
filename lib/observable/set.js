...
module.exports = function (observable) {
  ...
  observable.define({
    ...
    set (val, stamp, nocontext, escape, newobs) {
      var created
      if (stamp === void 0) {
        created = true
        stamp = vstamp.create()
      ...
      const base = set.call(this, val, stamp, nocontext, escape) || (newobs && this)
      if (stamp) {
        if (base) {
          let on = base.__on
          let emitter = on._data || on.data
          if (emitter) {
            emitter.emit(base, stamp, val)
          } else if (base._i) {
            emitinstances(base._i, stamp, val)
          }
        }
        if (created) {
          vstamp.close(stamp)
