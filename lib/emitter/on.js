...
var isObj = require('vigour-util/is/plainobj')
...
exports.define = {
  on (val, key, unique, stamp) {
    console.log('---> lets add some fuckeridoodles', val, key)
    // start at zero faster looping!
    if (!key) { key = id(this, val) }
    if (typeof val === 'function') {
      console.log('ADD', this.realPath())
...
function add (emitter, type, val, key, unique, stamp) {
  ...
  if (!emitterType) {
    if (stamp) {
      ...
      vstamp.on(stamp, () => {
        console.log('CRAPPER! go', stamp)
        // have to check again, in the mean time emitter might be created
        if (!emitter[type]) {
          console.log('make it!')
